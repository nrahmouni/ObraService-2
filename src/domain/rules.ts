...
export function generateDeliveryNotesFromReport(
  ...
  for (const [subcontractorId, entries] of entriesBySubcontractor.entries()) {
    ...
    const newNote: DeliveryNote = {
      ...
      totalHours,
      status: 'Pending',
      evidenceAttachments: report.evidenceAttachments || (report.evidenceUrls ? report.evidenceUrls.map((u, i) => ({
        id: `att_${i}`,
        url: u,
        caption: `Evidencia fotográfica ${i + 1}`,
