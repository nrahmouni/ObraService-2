...
export const DeliveryNotesView: React.FC<DeliveryNotesViewProps> = ({ state }) => {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'albaranes' | 'fichaje'>('albaranes');
  ...
  return (
    <div className="animate-in fade-in duration-500">
      ...
      <div className="flex gap-4 border-b border-slate-200 mb-6">
        ...
        <button
          onClick={() => setActiveSubTab('fichaje')}
          className={`pb-2.5 text-xs font-black uppercase tracking-wider transition-all border-b-2 ${
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
            <h3 className="text-xs font-black text-slate-900 uppercase tracking-tight mb-4 flex items-center gap-2">
              ...
              <span>Historial de Fichajes de la Jornada</span>
            </h3>
              ...
              if (logs.length === 0) {
                return (
                  <div className="text-center py-8 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                    ...
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Sin registros de fichajes</span>
                  </div>
                );
              }
                  ...
                  <table className="w-full text-left text-xs">
                    <thead>
                      <tr className="border-b border-slate-100 text-[9px] font-black text-slate-400 uppercase tracking-widest bg-slate-50/50">
                        ...
                        <th className="py-2 px-3">Obra / Destino</th>
                        <th className="py-2 px-3 text-center">Tipo</th>
                        <th className="py-2 px-3">Ubicación GPS / Distancia</th>
                      ...
                      {logs.sort((a,b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()).map(log => (
                        <tr key={log.id} className="hover:bg-slate-50/50 transition-colors">
                          <td className="py-3 px-3">
                            <span className="font-extrabold text-slate-950 block">{log.userNameSnapshot}</span>
                            <span className="text-[8px] font-bold text-slate-400 uppercase tracking-wider">
                              {log.userRoleSnapshot === 'SUBCONTRACTOR_USER' ? 'Operario' : 'Jefe de Obra'}
                            </span>
                          </td>
                          <td className="py-3 px-3">
                            <strong className="text-slate-800 uppercase font-black block">{log.projectNameSnapshot}</strong>
                          </td>
                          ...
                          <td className="py-3 px-3">
                            <div className="flex items-center gap-1.5 text-slate-600">
                              <Navigation className="w-3.5 h-3.5 text-[#FF6600] shrink-0" />
                              <span className="font-bold text-[10px] text-slate-700">
                                {log.lat.toFixed(5)}, {log.lng.toFixed(5)}
                              </span>
                              <span className="text-[9px] font-black text-emerald-600 uppercase tracking-tight bg-emerald-50 px-1 py-0.2 rounded">
                                a {log.distanceMeters.toFixed(1)}m
                              </span>
                            </div>
                          </td>
